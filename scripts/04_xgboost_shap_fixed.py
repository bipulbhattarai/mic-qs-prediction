...
def prepare_regression(df):
    ...
    # Prefer precomputed log target if present, otherwise derive it safely.
    if "mic_rate_log" in df_reg.columns and df_reg["mic_rate_log"].notna().all():
        y = df_reg["mic_rate_log"].values
    else:
        print("  mic_rate_log missing or incomplete; deriving with np.log1p(mic_rate)")
        y = np.log1p(df_reg["mic_rate"].astype(float).values)

    # Drop columns that are entirely missing to avoid shape mismatches after imputation.
    empty_cols = X.columns[X.isna().all()].tolist()
    if empty_cols:
        print(f"  Dropping fully empty regression columns: {empty_cols}")
...
def prepare_classification(df):
    ...
    empty_cols = X.columns[X.isna().all()].tolist()
    if empty_cols:
        print(f"  Dropping fully empty classification columns: {empty_cols}")
        X = X.drop(columns=empty_cols)
...
def plot_shap_beeswarm(model, X_te, feature_names, tag="reg"):
    ...
    # Normalize SHAP output shape across model/task variants
